package com.programming.techie.inventoryservice.config;
...
public class SwaggerConfig {
			...
			if (servers != null) {
				servers = servers.stream()
						.map(server -> {
							...
							server.setUrl("");
							server.description("Inventory API service");
							return server;
						})
    ...
    public OpenAPI customizeOpenAPI()
    {
        ...
        return new OpenAPI()
          ...
          .info(new Info().title("MSPT Inventory API")
          .description("OpenAPI definition for the MSPT Inventory API endpoints."));
    }
